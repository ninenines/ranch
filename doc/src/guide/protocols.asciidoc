...
it must call `ranch:handshake/{1,2}` or `ranch:accept_ack/1` before
doing any socket operation. This will ensure the connection process
...
calling `ranch:handshake/{1,2}` or `ranch:accept_ack/1`.
You can use `Transport:setopts/2` for that purpose.
...
you won't be able to call `ranch:handshake/{1,2}` or `ranch:accept_ack/1`
