...
ack_timeout (5000)::
	Maximum allowed time for the `ranch:handshake({1,2}` or `ranch:accept_ack/1` call to finish.
...
Either this or one of the `ranch:handshake/{1,2}`
functions MUST be used by a connection process to inform
...
Either one of the `ranch:handshake/{1,2}` or `ranch:accept_ack/1`
functions MUST be used by a connection process to inform
Ranch that it initialized properly and let it perform any
additional operations before the socket can be safely used.
