...
If the connection was created through a UNIX Domain socket,
`{local, PeerSocket}` is returned, where `PeerSocket` is
usually an empty binary.
...
If the connection was created through a UNIX Domain socket,
`{local, SocketFile}` will be returned instead.
